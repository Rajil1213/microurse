<template>
  <div class="text-center">
    ...
    <hr class="my-5" />
    <p v-if="noComments" class="m-2 text-xs italic">No Comments</p>
    <div v-else>
      ...
      <li class="comments-list" :key="commentKey">
        <ul v-for="comment in comments" class="comment-card" :key="comment.id">
          <h4 class="text-sm" :class="statusToColor(comment.status)">
...
const fetchComments = async () => {
  try {
    const res = await backendClient.get<Array<Comment>>(`/posts/${props.postId}/comments`);
    if (res.status === 200) {
...
<style scoped>
.comments-list {
  @apply m-2 my-5 max-h-[4rem] list-none overflow-scroll bg-slate-300;
}

.comment-card {
  @apply m-2 h-fit bg-teal-100 p-1 shadow-md;
}
</style>
